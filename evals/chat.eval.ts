...
import { BAR, belowBar, mentioned, percent, scoreFields } from "./score"

// AI evals (spec §6; T20, then all 11 agreements in T30): the real model,
// through the real chat. The bars are in score.ts; the report lands in
// evals/report.md.

const today = Temporal.Now.plainDateISO().toString()
/**
 * A whole agreement takes a few questionnaires (the CSA has 25 required
 * fields, up to 5 questions a card). The slowest case (the DPA) took 12
 * turns; more than 16 means the chat is stuck, and the draft fails the
 * "finished" bar.
 */
const MAX_TURNS = 16
...
afterAll(() => {
  ...
  expect(belowBar(summary), "below the bar (see evals/report.md)").toEqual([])
})
...
| Measure | Result | Bar | |
|---|---|---|---|
${row("Right agreement", percent(summary.documents), `≥ ${percent(BAR.documents)}`, summary.documents >= BAR.documents)}
${row("Right field values", percent(summary.fields), `≥ ${percent(BAR.fields)}`, summary.fields >= BAR.fields)}
${row("Invalid writes (refused values and failed tool calls)", String(summary.invalidWrites), String(BAR.invalidWrites), summary.invalidWrites === BAR.invalidWrites)}
${row("Drafts finished (markComplete said complete)", percent(summary.finished), percent(BAR.finished), summary.finished >= BAR.finished)}
...
${conversations
  .map(
    (each) =>
      `| ${each.kind} | ${each.name} | ${outcome(each)}${each.invalidWrites ? `, ${each.invalidWrites} failed writes` : ""} | ${each.turns} | ${each.usage.calls} | ${each.usage.input} (${each.usage.cached}) | ${each.usage.output} | ${dollars(cost(each.usage))} |`
  )
  .join("\n")}
${misses()}`
}

/** What a conversation came to, in a few words for its table row. */
function outcome(each: Result) {
  const got = name(each.document?.got ?? null)
  if (each.kind === "guardrail")
    return `${mark(each.guardrail?.ok ?? false)} ${each.guardrail?.why}`
  if (each.kind === "choose") {
    const ok = each.document?.ok ?? false
    const want = ok ? "" : ` (want ${name(each.want)})`
    const suggested = !each.suggested
      ? ""
      : each.suggested.ok
        ? `, named ${each.suggested.named.join(", ")}`
        : ", named no related agreement"
    return `${mark(ok)} ${got}${want}${suggested}`
  }
  const fields = each.fields ?? []
  const right = fields.filter((field) => field.ok).length
  const ok = (each.document?.ok ?? false) && right === fields.length
  return `${mark(ok)} ${got}, ${right}/${fields.length} fields, ${each.complete ? "complete" : "not complete"}`
}
