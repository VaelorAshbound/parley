export const meta = {
  name: 'par1-wave-c',
  description: 'PAR-1 wave C: build, review, fix+simplify T23b, T26, T31, T37 in parallel worktrees',
  phases: [
...
// Workflow script for wave C (see plan.md "Parallel run"). A new session:
...
const BASE = '5d273a6'
const TASKS = [
  { id: 'T23b', port: 3131, reviewSkills: 'agent-skills:security-and-hardening and two-factor-authentication-best-practices, since this is auth', focus: 'two-factor authentication (TOTP + backup codes) through Better Auth\'s twoFactor plugin, a "Two-factor" card in Settings, and the sign-in challenge. Auth code: security matters most. Keep the T23 note: a reset of an unconfirmed account turns two-factor off (claimUnconfirmedAccount).' },
  { id: 'T26', port: 3132, reviewSkills: 'agent-skills:security-and-hardening and agent-skills:doubt-driven-development, since this is billing', focus: 'Polar sandbox billing for Parley Pro: checkout, webhooks (verified signatures, idempotent), plan state, /pricing, and planOf/tierOf returning "pro" for Pro users (T27 made DAILY_MESSAGES.pro and the Get Pro link ready). Billing: load agent-skills:doubt-driven-development. Sandbox only, never Polar production. A webhook endpoint for a Preview or `polar listen` is a dev/test resource.' },
  { id: 'T31', port: 3133, reviewSkills: 'agent-skills:code-review-and-quality only (read the PDF/DOCX checks closely)', focus: 'real export of all 12 documents with visual checks of each PDF and DOCX. Look at the T24 note about static brand fonts for the PDF (real TrueType subsets instead of Type 3). Browser Run is a real cloud cost: keep the number of real PDF runs small.' },
  { id: 'T37', port: 3134, reviewSkills: 'web-design-guidelines, since this is UI polish', focus: 'empty-state and first-run polish, matching brand.md and the design sources in work/PAR-1/design/. Also the phone design\'s bottom Share + Download bar (from T25\'s notes). Look at screenshots in light and dark at 1440 and 375 px.' },
]
...
- Scope: only ${t.id}. Other tasks (T23b, T26, T31, T37) are being built at the same time in other worktrees; do not do their work. If you need something from another task, note it in openIssues. New DB migrations and new ADR files are fine; the lead renumbers them at merge.
...
${RULES(t)}
Steps:
1. Setup: the harness may make your worktree from main, not our branch. First run \`git log --oneline -1\`; if HEAD is not ${BASE}, run \`git reset --hard ${BASE}\` (only on your own new branch, before any work). Then copy ${REPO}/.env to the worktree root and ${REPO}/apps/web/.dev.vars to apps/web/.dev.vars (never commit them). Run \`flock ${LOCK} pnpm install --frozen-lockfile\`.
