...
- **Typed errors.** Procedures declare `.errors({ NOT_FOUND, QUOTA_EXCEEDED, DAILY_LIMIT, TOO_MANY_REQUESTS, DRAFT_LIMIT, PRO_REQUIRED })` (T27: `TOO_MANY_REQUESTS` is the code oRPC's rate-limit middleware throws, so it replaces the planned `RATE_LIMITED`). The client checks them with `isDefinedError`, so every limit or paywall state has typed UI, not string matching.
...
- **Rate limit.** `CloudflareRatelimiter` wraps the Workers binding and is used through oRPC's rate-limit middleware, per user (T27). On oRPC 1.x they ship as `@orpc/experimental-ratelimit` (pinned with oRPC); `@orpc/cloudflare` is oRPC 2 (beta). No headers plugin: the binding answers only "allowed or not", so there are no `RateLimit-*` numbers to send. The daily AI budget stays our own Postgres counter.
