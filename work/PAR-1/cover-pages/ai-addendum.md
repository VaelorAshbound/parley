...
5. **Training Data and Training Purposes must agree on the finished page.** 1.3 allows training only when the page names both. So "Training Data: Usage Data" with "Training Purposes: None" would read like a grant that 1.3 then takes away. The finished page needs both set, or both None (an Other training data counts as set). Drafts can fill the two rows in any order.
...
- Fixed after T8: the registry typing gap (see `csa.md`), and "required when" through rules that run on the finished page only (call 5).
