...
- [x] **T27: Turnstile, rate limits, AI budgets** (M)
  ...
  - Decisions:
    ...
    - **UI:** the chat shows a calm note instead of "couldn't answer" (`limit-banner.tsx`, `limit-problem.ts`): a guest gets "Create a free account for 100 a day" (back to the draft), a free user when more come (their local time) and "Get Pro" (`/pricing`, T26), a burst "wait a few seconds" with Try again. A download past its limit says "wait a minute". `DAILY_MESSAGES` lives in `src/lib/limits.ts` for both sides.
  - Found and fixed on the way:
