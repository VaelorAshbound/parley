...
- [x] **T27: Turnstile, rate limits, AI budgets** (M)
  ...
  - Decisions:
    ...
    - **New guests per network:** Better Auth's limiter on `/sign-in/anonymous`: production 10, then none until an hour after the last. Test deployments (Turnstile test keys: local dev, Worker tests, Previews) 30 per 10 s, since e2e makes a guest per test from one address. It counts before the Turnstile check (Better Auth's order), so failed bots on a shared network use it up too; accepted.
  ...
  - For later:
    ...
    - Previews use Turnstile's test keys, so a bot passes there; the per-guest daily limit and per-network cap (30/10 s) bound it, and the OpenRouter key's monthly credit limit is the backstop (owner).
