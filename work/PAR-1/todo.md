...
- [x] **T27: Turnstile, rate limits, AI budgets** (M)
  - Done 2026-09-25. Skills: build, incremental-implementation, test-driven-development, source-driven-development, security-and-hardening, observability-and-instrumentation, git-workflow-and-versioning; cloudflare:turnstile-spin, cloudflare:workers-best-practices, better-auth-security-best-practices. Checked:
    - `pnpm check`; `pnpm test` (1051, 1 skipped); `pnpm test:workers` (318, new `limits.test.ts`: every limit at its edge); db tests for `withinDraftLimit` and `claimMessage` (incl. two connections racing for the last message).
    - e2e Chromium + Firefox on port 3132, the guest-based specs (limits, shell, auth, drafts, editing, export, smoke): 56/68 at load 10–12, then the 12 left (8 timeouts / Firefox NS_BINDING_ABORTED, 4 not run) 12/12 with `--last-failed --workers=2`. New `limits.spec.ts`: a guest's second draft asks for an account.
    - Resend: no sends. Cloud: nothing created (Rate Limiting namespaces are only config).
  - Decisions:
    - **Guests:** `/sign-in/anonymous` is in the captcha endpoints (same widget, action `auth`). The home page renders the widget for a first visit (interaction-only, so nothing shows unless Cloudflare wants a click); `signInGuest(captcha)` sends a fresh token on each try and throws a typed `GuestSignInError` (human-check / busy / failed) that the page turns into words.
    - **New guests per network:** Better Auth's limiter on `/sign-in/anonymous`: production 10, then none until an hour after the last (not "10 an hour": a network with a new guest every hour, like an office or a carrier NAT, stays locked after its 10th; accepted, Turnstile is the main gate and an account has no such cap). Previews 5 per 10 s: they pass every Turnstile token and their URL is guessable (review). Local dev and Worker tests 30 per 10 s, since e2e makes a guest per test from one address; the e2e fixture waits out 429s (up to 10 tries, its own 60 s timeout). It counts before the Turnstile check (Better Auth's order), so failed bots on a shared network use it up too; accepted.
    - **A guest keeps one draft:** `drafts.create` and `drafts.duplicate` answer typed `DRAFT_LIMIT` (403), under a per-user advisory lock (`withinDraftLimit`), so two at once make one. The home page says "Guests keep one draft" with "Create an account". Accounts have no limit.
    - **Rate Limiting bindings, per user** through oRPC's middleware and `CloudflareRatelimiter` (`@orpc/experimental-ratelimit` 1.15.4, pinned with oRPC; `@orpc/cloudflare` is oRPC 2 beta): every procedure 300/60 s (in `authed`, so SSR loads count too), AI routes 10/10 s, `export.pdf`/`export.docx` 10/60 s (T24 Must). Past one: typed `TOO_MANY_REQUESTS` (429), the code oRPC's middleware throws, instead of the planned `RATE_LIMITED` (spec §5 updated). The AI's tool calls count once with their turn (the middleware's dedupe). Previews have their own namespaces. `RATE_LIMITS` in `server/limits.ts` mirrors wrangler.jsonc; a unit test keeps them equal. **No headers plugin:** the binding only answers "allowed or not", so there are no `RateLimit-*` numbers to send.
    - **Daily messages** (guest 20, free 100, Pro 500 for T26) per UTC day in `ai_usage`: a message or a set of answers is one. `claimMessage` is one upsert that refuses past the limit, run in the chat's locked transaction after the message is accepted, so a refused turn saves and counts nothing. Typed `DAILY_LIMIT` (429) carries limit, tier and `resetsAt`. Each reply's tokens and cost are added after the response (`addAiUsage`). Logs: one `ai_limit` line per refusal (IDs and numbers), `ai_usage_failed` if the write fails; rate-limit refusals show as `request` lines with status 429.
    - **UI:** the chat shows a calm note instead of "couldn't answer" (`components/problem-note.tsx`, shared with the home page's start problems; `limit-problem.ts`): a guest gets "Create a free account for 100 a day" (back to the draft), a free user when more come (their local time) and "Get Pro" (`/pricing`, T26), a burst "wait a few seconds" with Try again. A download past its limit says "wait a minute". `DAILY_MESSAGES` lives in `src/lib/limits.ts` for both sides.
  - Found and fixed on the way:
    - **PAR-11:** `useTurnstile().headers()` now resets the handle as it is after the wait (`ref.current?.reset()` in `finally`); a browser test loads a stand-in Turnstile late and checks the second send gets a new token.
...
- [x] **T28: Cron: purge old guest data** (S)
  - Done 2026-09-25. Skills: build, incremental-implementation, test-driven-development, source-driven-development (Cloudflare docs: scheduled handler, Cron Triggers, Previews and crons, `createScheduledController`), doubt-driven-development, git-workflow-and-versioning, observability-and-instrumentation, documentation-and-adrs; cloudflare:wrangler, cloudflare:workers-best-practices, neon:neon-postgres-egress-optimizer. Checked:
    - `pnpm check`; `pnpm test` (1033); `pnpm test:workers` (309, 8 new in `cron.test.ts`); e2e `smoke.spec.ts` on Chromium + Firefox, PORT=3133 (6; T28 has no UI, the smoke run checks the changed Worker entry).
    - A real `pnpm dev`: `/cdn-cgi/local/scheduled` with `time` set to 2020 (so nothing in the shared dev database could match) ran `server.ts`'s `scheduled()`: outcome ok, one `purge_done` line.
    - Every guard in `deleteIdleGuests` was removed in turn (10 mutations), and a test failed each time.
    - Doubt review was the skill's degraded self-review: a subagent can't start a fresh-context reviewer. Cross-model was skipped (non-interactive run).
  - Built:
    - **`packages/db` `deleteIdleGuests` / `deleteExpiredSessions`**: one bounded `DELETE … WHERE id IN (SELECT … LIMIT n FOR UPDATE SKIP LOCKED)` each, returning only a row count (no rows cross the wire, for Neon egress). The foreign keys cascade a guest's drafts, chats, AI usage and sessions.
    - **`server/cron.ts`**: `scheduled()` takes its time from `controller.scheduledTime` (the fake clock in tests), deletes in batches of 500, at most 20 a run per kind. It logs `purge_done` (`guests`, `sessions`, `complete`, `durationMs`), or `purge_failed` with the error code, and throws, so Cron Events show a failed run.
  - Decisions:
    - **A guest is idle** when the user row, every session (used or still valid), every draft (a chat message touches it) and AI usage that day are all older than 7 days. A still-valid session keeps a guest, because its cookie can come back. So only guests who can't come back go (Better Auth sessions last 7 days from their last daily refresh).
