...
- **A `counted_export` table**, one row per counted document: `user_id` (cascades with the user), `draft_id` (no foreign key, on purpose), `document_id` (the agreement counted), `counted_at`, an index on `(user_id, counted_at)`, and a unique index on `(draft_id, document_id)`, so one draft's agreement can never be counted twice. The monthly count is `count(*)` of the user's rows since the start of the UTC month. Deleting a draft keeps its row, so its place is not given back. `draft.firstExportedAt` stays the "already counted" flag for re-exports.
...
- **Choosing another agreement sets `firstExportedAt` from that agreement's row** (`drafts.chooseDocument`, `countedAt`): an agreement this draft was never downloaded as is a new document (null, counted on its next download); one it was downloaded as keeps its count, so switching away by mistake and back is still free. The old count stays.
...
- Migration `0002_counted_export` is additive (a new table and two indexes). The lead applies it to Neon after merge.
